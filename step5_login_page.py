...
export default function LoginPage() {
  ...
  const handleCompleteSocialAuth = (e?: React.FormEvent) => {
    ...
    if (!chosenEmail || !chosenEmail.includes('@')) {
      setError('Valid social account email is required.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dynamic Social Buttons based on Config */}
        {(config.googleEnabled || config.facebookEnabled || config.appleEnabled) && (
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: isDayMode ? '#94a3b8' : '#64748b' }}>or email</span>
              <div className="h-px flex-1" style={{ backgroundColor: isDayMode ? '#e2e8f0' : '#1e293b' }} />
            </div>
          </div>
        )}
