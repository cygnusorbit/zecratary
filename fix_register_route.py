...
# 1. Identify the active Next.js app directory
app_dir = None
matches = glob.glob('**/admin/social-login-setting/page.tsx', recursive=True)
if matches:
    app_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(matches[0]))))

if not app_dir:
    for candidate in ['src/app', 'apps/web/src/app', 'app']:
...
print(f"✓ Found active app directory: {app_dir}")

# 2. Determine if the project uses an (auth) route group or direct routes
login_matches = glob.glob(f"{app_dir}/**/login/page.tsx", recursive=True)
use_auth_group = False
if login_matches:
    login_dir = os.path.dirname(login_matches[0])
    if "(auth)" in login_dir:
        use_auth_group = True

if use_auth_group:
    target_dir = os.path.join(app_dir, '(auth)', 'register')
    conflict_dir = os.path.join(app_dir, 'register')
else:
    target_dir = os.path.join(app_dir, 'register')
    conflict_dir = os.path.join(app_dir, '(auth)', 'register')

# Remove duplicate conflicting folder to avoid Next.js routing collisions
if os.path.exists(conflict_dir):
    print(f"🧹 Removing conflicting duplicate folder: {conflict_dir}")
    shutil.rmtree(conflict_dir)

os.makedirs(target_dir, exist_ok=True)
register_page_path = os.path.join(target_dir, 'page.tsx')

# 3. Write functional /register page
...
export default function RegisterPage() {
  ...
  const startSocialAuth = (provider: SocialProvider) => {
    setError('');
    if (provider === 'google' && !socialConfig.googleEnabled) return setError('Google login is disabled.');
    if (provider === 'facebook' && !socialConfig.facebookEnabled) return setError('Facebook login is disabled.');
    if (provider === 'grok' && !socialConfig.grokEnabled) return setError('Grok login is disabled.');
      ...
      >
        ...
        {hasAnySocialProvider && (
          <div className="space-y-2.5">
            ...
            <div className={`grid ${socialConfig.facebookEnabled && socialConfig.grokEnabled ? 'grid-cols-2' : 'grid-cols-1'} gap-2.5`}>
              {socialConfig.facebookEnabled && (
                <button
                  type="button"
                  disabled={loading || socialLoading !== null}
...
print(f"✓ Installed active register page at: {register_page_path}")
