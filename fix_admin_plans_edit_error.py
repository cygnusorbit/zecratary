...
export default function AdminSubscriptionPlans() {
  ...
  const handleEditPackage = (pkg: SubscriptionPackageConfig) => {
    ...
    // Safely extract featuresText from string or array
    ...
    // Safely extract allowedAiModels as a comma-separated string
  ...
  // Safe multi-fallback resolution for current features list
  const currentFeaturesList = (typeof form.featuresText === 'string'
    ? form.featuresText
    : (Array.isArray((form as any).features) ? (form as any).features.join('\n') : '')
  ).split('\n').map(s => s.trim()).filter(Boolean);

  // Safe resolution for current allowed models
