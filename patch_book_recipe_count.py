...
export default function SavedRecipesPage() {
  ...
  const handleAssignToBook = (bookId: string) => {
    ...
    // Update book recipe counts dynamically in localStorage
    const storedBooks = JSON.parse(localStorage.getItem('zecratary_recipe_books') || '[]');
    const updatedBooks = storedBooks.map((b: any) => {
      if (b.id === bookId) {
        return { ...b, recipeCount: (b.recipeCount || 0) + 1 };
      }
      return b;
    });
                  ...
                  <div className="px-6 grid grid-cols-1 sm:grid-cols-4 gap-3">
                    <div className="relative">
                      ...
                      >
                        ...
                        {books.map(b => (
                          <option key={b.id} value={b.id}>{b.title} ({b.recipeCount || 0})</option>
                        ))}
                      </select>
                    </div>
...
print("✅ Cookbook recipe count tracking successfully updated!")
