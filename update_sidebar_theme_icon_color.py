...
# 2. Update lib/themeConfig.ts with sidebarIconColor support
...
# 3. Update app/admin/page.tsx with the Sidebar Icon Color picker in the Theme Tab
...
# 4. Find and dynamically patch sidebar navigation files to link with var(--color-sidebar-icon)
sidebar_candidates = []
for root, dirs, files in os.walk('.'):
    if any(ignore in root for ignore in ['node_modules', '.next', '.git', 'dist', 'build']):
        continue
    for file in files:
        if file.endswith('.tsx') or file.endswith('.jsx'):
            ...
            try:
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    # Identify files that define sidebar nav items (Pantry, Chef, Shopping, Books, etc.)
                    if ('Pantry' in content or 'Chef' in content or 'Books' in content) and ('Dashboard' in content or 'Shopping' in content or 'Planner' in content):
                        sidebar_candidates.append(filepath)
            except Exception:
                pass

for s_path in sidebar_candidates:
    try:
        with open(s_path, 'r', encoding='utf-8') as f:
            code = f.read()
        
        # Replace static emerald classes on icons with dynamic CSS variable
        original_code = code
        code = re.sub(r'\btext-emerald-400\b', 'text-[var(--color-sidebar-icon,#10b981)]', code)
        code = re.sub(r'\btext-emerald-500\b', 'text-[var(--color-sidebar-icon,#10b981)]', code)
        code = re.sub(r'\btext-teal-400\b', 'text-[var(--color-sidebar-icon,#10b981)]', code)
        
        if code != original_code:
            with open(s_path, 'w', encoding='utf-8') as f:
                f.write(code)
            print(f"✓ Connected sidebar nav icon styles to var(--color-sidebar-icon) in: {s_path}")
    except Exception as e:
        print(f"Notice: Failed to patch {s_path}: {e}")

# 5. Ensure global CSS rule fallback is registered
...
print("\n🚀 Sidebar icon color customizer installed successfully!")
