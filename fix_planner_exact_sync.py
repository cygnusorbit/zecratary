...
export default function PlannerPage() {
  ...
  // Fetch true live saved recipes from all possible keys
  const fetchLiveSavedRecipes = () => {
    const possibleKeys = ['zecratary_recipes', 'saved_recipes', 'recipes', 'foodieprep_recipes'];
  ...
  useEffect(() => {
    ...
    fetchLiveSavedRecipes();
  }, []);

  const openAddModal = (date: string) => {
    // Re-read storage at the exact moment modal is opened
    fetchLiveSavedRecipes();
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 pr-1 pt-1">
                  {filteredSavedRecipes.length === 0 ? (
                    <div className="py-8 text-center text-slate-500">
                      No saved recipes found. Save or create recipes on the Saved Recipes page first!
                    </div>
                  ) : (
...
print("✅ Planner page updated to load live Saved Recipes dynamically!")
