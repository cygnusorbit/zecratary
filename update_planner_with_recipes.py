...
export default function PlannerPage() {
  ...
  const handleAddMealSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedRecipeObj) {
      alert('Please select a recipe from your saved collection.');
      return;
    }
    const newMeal = {
      id: 'plan_' + Date.now(),
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 text-slate-100 pb-24 px-4">
      ...
      {/* ADD MEAL MODAL WITH SAVED RECIPES PICKER & THUMBNAILS */}
      {showAddMealModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111726] border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-5 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <Plus className="h-5 w-5 text-[#E05638]" /> Add Recipe from Collection
            </h2>
            ...
            <form onSubmit={handleAddMealSubmit} className="space-y-4 text-xs">
              ...
              <div className="space-y-2">
                <label className="block text-slate-400 font-semibold">Select Saved Recipe *</label>
                ...
                {/* RECIPE SELECTION LIST WITH THUMBNAILS */}
                <div className="max-h-52 overflow-y-auto space-y-2 pr-1 pt-1">
                  {filteredSavedRecipes.length === 0 ? (
                    <div className="py-8 text-center text-slate-500">No matching saved recipes found.</div>
                  ) : (
...
print("✅ Planner page updated with Saved Recipes selector and thumbnails!")
