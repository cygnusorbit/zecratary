...
# 1. Update/Ensure Recipes Page writes to 'zecratary_recipes' and dispatches storage events
recipes_paths = ["apps/web/src/app/recipes/page.tsx", "apps/web/src/app/saved/page.tsx"]

for r_path in recipes_paths:
    if os.path.exists(r_path):
        with open(r_path, "r", encoding="utf-8") as f:
            r_code = f.read()
        
        # Ensure consistent localStorage key
        r_code = re.sub(r"localStorage\.(?:getItem|setItem)\(['\"][^'\"]*recipes[^'\"]*['\"]", "localStorage.setItem('zecratary_recipes'", r_code)
        
        # Dispatch event on save
        if "window.dispatchEvent(new Event('zecratary_recipes_updated'))" not in r_code:
            r_code = r_code.replace(
                "localStorage.setItem('zecratary_recipes', JSON.stringify(updated));",
                "localStorage.setItem('zecratary_recipes', JSON.stringify(updated));\n    if (typeof window !== 'undefined') window.dispatchEvent(new Event('zecratary_recipes_updated'));"
            )
            with open(r_path, "w", encoding="utf-8") as f:
                f.write(r_code)
            print(f"✅ Updated event dispatcher in {r_path}")

# 2. Re-write Planner Page to reliably fetch and sync live saved recipes
planner_path = "apps/web/src/app/planner/page.tsx"
...
export default function PlannerPage() {
  ...
  // Function to load recipes strictly from local storage
  const loadSavedRecipes = useCallback(() => {
    if (typeof window === 'undefined') return;
    try {
      const raw = localStorage.getItem('zecratary_recipes');
      if (raw) {
    ...
    setSavedRecipes(defaults);
    localStorage.setItem('zecratary_recipes', JSON.stringify(defaults));
  }, []);
  ...
  useEffect(() => {
    ...
    // Listen for custom recipe updates & storage events
    window.addEventListener('zecratary_recipes_updated', loadSavedRecipes);
    window.addEventListener('storage', loadSavedRecipes);
    return () => {
      window.removeEventListener('zecratary_recipes_updated', loadSavedRecipes);
...
print("✨ Successfully synchronized Saved Recipes storage across Recipes and Planner!")
