...
# 1. Update/Implement server API route /api/admin/plans/route.ts
api_candidates = [
...
api_paths = [p for p in api_candidates if os.path.exists(p)]
if not api_paths:
    matches = glob.glob('**/api/admin/plans/route.ts', recursive=True)
    api_paths = [p for p in matches if 'node_modules' not in p and '.next' not in p]

if not api_paths:
    # If no route file exists, create it at default location
    default_api = 'src/app/api/admin/plans/route.ts' if os.path.exists('src') else 'app/api/admin/plans/route.ts'
    os.makedirs(os.path.dirname(default_api), exist_ok=True)
    api_paths = [default_api]

api_route_code = r'''// Generated / Updated by AI Collaborator
...
for p in api_paths:
    ...
    print(f"✓ Updated API Route with permanent server DELETE handling: {p}")

# 2. Update admin/plans/page.tsx with robust deletion and tombstone tracking
...
admin_page_paths = [p for p in admin_page_candidates if os.path.exists(p)]
if not admin_page_paths:
    matches = glob.glob('**/admin/plans/page.tsx', recursive=True)
    admin_page_paths = [p for p in matches if 'node_modules' not in p and '.next' not in p]

clean_admin_page = r'''// Generated / Updated by AI Collaborator
'use client';
...
for p in admin_page_paths:
    with open(p, 'w', encoding='utf-8') as f:
        f.write(clean_admin_page)
    print(f"✓ Deployed plan deletion with tombstone filtering in: {p}")

print("\n🚀 Successfully deployed permanent plan deletion for /admin/plans!")
