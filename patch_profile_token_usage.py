...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const applySavedTheme = () => {
      try {
        ...
        // Load Token Usage
        const tokenRaw = localStorage.getItem('zecratary_token_usage');
        if (tokenRaw) {
          const parsedTokens = JSON.parse(tokenRaw);
          setTokenUsage(prev => ({ ...prev, ...parsedTokens }));
        }
      } catch (e) {}
    };
    ...
    window.addEventListener('zecratary_token_usage_updated', applySavedTheme);
    ...
    return () => {
      ...
      window.removeEventListener('zecratary_token_usage_updated', applySavedTheme);
        ...
        if (rawPlans) {
          ...
          if (Array.isArray(directPlans) && directPlans.length > 0) {
            directPlans.forEach((p: any) => {
              const isFreePlan = p.priceCents === 0 || p.isFree;
              const pf = (Array.isArray(p.features) && p.features.length >= (isFreePlan ? 7 : 3)) ? p.features : (isFreePlan ? DEFAULT_TASTER_FEATURES : DEFAULT_PRO_FEATURES);
              plansMap.set(p.slug, {
                ...p,
                features: pf,
                priceFormatted: isFreePlan ? 'Free' : `${currencySymbol}${(p.priceCents / 100).toFixed(2)} / ${(p.interval || 'MONTH').toLowerCase()}`
              });
            });
          }
        }
  ...
  const handleSelectPlan = async (plan: SubscriptionPlanItem) => {
    ...
    try {
      ...
      const isUpgradeToAnnual = plan.interval === 'YEAR' && currentPlanSlug.includes('monthly');
      const isDowngradeToMonthly = plan.interval === 'MONTH' && currentPlanSlug.includes('annual');

      let actionMsg = isFree 
      ...
      if (isUpgradeToAnnual) {
        actionMsg = `Upgraded to ${plan.name} (Annual)! Previous Monthly plan cancelled. Renewal date: ${new Date(newExpiryDate).toLocaleDateString()}.`;
      } else if (isDowngradeToMonthly) {
        actionMsg = `Switched to ${plan.name} (Monthly)! Previous Annual plan cancelled. Renewal date: ${new Date(newExpiryDate).toLocaleDateString()}.`;
      }
  ...
  const tokenPercentage = Math.min(Math.round((tokenUsage.totalTokens / tokenUsage.monthlyLimit) * 100), 100);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {user.role === 'admin' && (
          <div className="flex items-center gap-2">
            ...
            >
              <CreditCard className="h-3.5 w-3.5 text-blue-400" /> {t('paymentGateway') || 'Payment & Gateway'}
            </Link>
            <Link
              href="/admin/plans"
              className="border font-bold text-xs px-3.5 py-2 rounded-xl transition flex items-center gap-1.5"
        ...
        >
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-baseline">
                <span className="text-xs text-slate-400 font-medium">Monthly Token Consumption</span>
                <span className="text-sm font-black text-white font-mono">
                  {tokenUsage.totalTokens.toLocaleString()} <span className="text-[11px] text-slate-500">/ {tokenUsage.monthlyLimit.toLocaleString()}</span>
                </span>
              </div>
              ...
              <div className="w-full bg-slate-900 border border-slate-800 rounded-full h-3.5 overflow-hidden p-0.5">
                <div 
                  ...
                  style={{ 
                    ...
                    backgroundColor: tokenPercentage > 85 ? '#ef4444' : tokenPercentage > 60 ? '#f59e0b' : 'var(--color-primary, #E05638)'
                  }}
                />
              </div>
              <div className="flex justify-between text-[11px] text-slate-500">
                <span>{tokenPercentage}% of monthly limit used</span>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              ...
              >
                <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block">Prompt Input[cite: 3]</span>
                <span className="text-base font-black text-white font-mono">{tokenUsage.promptTokens.toLocaleString()}</span>
                <span className="text-[10px] text-slate-500 block">Tokens (Instructions & History)</span>
              </div>
              ...
              >
                <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block">Completion Output[cite: 3]</span>
          ...
          >
            <div className="flex items-center gap-1.5 font-bold text-slate-300">
              <Activity className="h-3.5 w-3.5 text-orange-400" /> Gemini API & Token Tracking[cite: 3]
            </div>
            <p className="leading-relaxed text-slate-400">
              Tokens are tracked automatically via response metadata (`usageMetadata`) whenever you chat with the AI assistant or generate recipes.
            </p>
          </div>
        </div>
