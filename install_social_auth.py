...
# 1. CREATE SOCIAL AUTH SERVICE (lib/socialAuth.ts)
...
print(f"✓ Installed social auth engine at: {social_auth_path}")

# 2. UPDATE /register/page.tsx
register_dirs = [
    os.path.join(app_dir, 'register'),
    os.path.join(app_dir, '(auth)', 'register')
]
...
print(f"✓ Installed social auth on Register at: {register_page_path}")
...
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { Mail, Lock, CheckCircle2, AlertCircle, ArrowRight, Sparkles, Eye, EyeOff, ShieldCheck, Loader2 } from 'lucide-react';
import { getCurrentUser, setCurrentUser, initAuthStorage, User } from '@/lib/auth';
import { executeSocialAuth, SocialProvider } from '@/lib/socialAuth';
import { useTranslation } from '@/components/LanguageProvider';
    ...
    >
      ...
      >
        <div className="space-y-1.5 text-center">
          ...
          <p className="text-xs" style={{ color: isDayMode ? '#64748b' : 'var(--color-text-secondary, #94a3b8)' }}>
            Sign in to access your saved recipes, meals, and analytics.
          </p>
        </div>
...
with open(login_page_path, 'w', encoding='utf-8') as f:
    f.write(login_code)
print(f"✓ Installed social auth on Login at: {login_page_path}")

print("Social login installation complete!")
