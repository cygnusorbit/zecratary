          ...
          <div className="bg-[#111726] border border-slate-800 rounded-3xl max-w-4xl w-full max-h-[92vh] flex flex-col overflow-hidden shadow-2xl relative">
            ...
            <div className="overflow-y-auto flex-1 space-y-6">
              ...
              {!isEditing ? (
                ...
                /* FULL EDIT FORM WITH DRAG/MOVE REORDER CONTROLS */
                <div className="p-6 space-y-6">
                  ...
                  {/* INGREDIENTS EDIT TAB WITH DRAG/GRIP & MOVE BUTTONS */}
                  {editTab === 'ingredients' && (
                    <div className="space-y-4 text-xs">
                      ...
                      <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                        {Array.isArray(editForm.ingredients) && editForm.ingredients.map((ing: any, idx: number) => (
                          <div key={idx} className="flex items-center gap-2 bg-[#0B101D] p-2.5 rounded-xl border border-slate-800 group">
                            {/* Grip & Move Controls */}
                            <div className="flex flex-col gap-0.5 text-slate-500">
                              <button onClick={() => moveIngredient(idx, 'up')} disabled={idx === 0} className="hover:text-white disabled:opacity-30" title="Move Up">
                                <ArrowUp className="h-3 w-3" />
                              </button>
                              <button onClick={() => moveIngredient(idx, 'down')} disabled={idx === editForm.ingredients.length - 1} className="hover:text-white disabled:opacity-30" title="Move Down">
                            ...
                            <input
                              ...
                              className="w-14 bg-slate-900 border border-slate-800 rounded-lg p-2 text-xs text-white text-center font-bold outline-none"
                            />
                            <input
                              type="text"
                              placeholder="g"
                              ...
                              className="w-16 bg-slate-900 border border-slate-800 rounded-lg p-2 text-xs text-slate-300 text-center outline-none"
                            />
                            <input
                              type="text"
                  ...
                  {/* STEPS EDIT TAB WITH MOVE CONTROLS */}
                  {editTab === 'steps' && (
                    <div className="space-y-4 text-xs">
                      <div className="flex justify-between items-center">
                      ...
                      <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                        {Array.isArray(editForm.instructions) && editForm.instructions.map((step: string, idx: number) => (
                          <div key={idx} className="flex items-center gap-3 bg-[#0B101D] p-3 rounded-xl border border-slate-800">
                            <div className="flex flex-col gap-0.5 text-slate-500">
                              <button onClick={() => moveStep(idx, 'up')} disabled={idx === 0} className="hover:text-white disabled:opacity-30" title="Move Up">
                                <ArrowUp className="h-3 w-3" />
                              </button>
                              <button onClick={() => moveStep(idx, 'down')} disabled={idx === editForm.instructions.length - 1} className="hover:text-white disabled:opacity-30" title="Move Down">
                                <ArrowDown className="h-3 w-3" />
                              </button>
                            </div>

                            <span className="w-6 h-6 rounded-full bg-[#E05638]/20 text-[#E05638] font-bold flex items-center justify-center shrink-0">
                              {idx + 1}
                            </span>
                            <textarea
                              rows={2}
...
print("✅ Ingredient drag/move reorder controls added successfully!")
