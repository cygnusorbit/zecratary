...
export default function LoginPage() {
  ...
  const handleSocialClick = (provider: SocialProvider) => {
    ...
    // Force redirection to Google OAuth Account Selector
    if (provider === 'google' && config.googleClientId && typeof window !== 'undefined') {
    ...
    // Default modal fallback if credentials are unset or provider is Facebook/Apple
...
print(f"✓ Patched /login with Google OAuth account selector: {login_page_path}")

# 3. Patch /register/page.tsx with the same account selector redirect
register_dirs = [os.path.join(app_dir, 'register'), os.path.join(app_dir, '(auth)', 'register')]
register_dir = next((d for d in register_dirs if os.path.exists(d)), register_dirs[0])
register_page_path = os.path.join(register_dir, 'page.tsx')

if os.path.exists(register_page_path):
    with open(register_page_path, 'r', encoding='utf-8') as f:
        reg_code = f.read()

    # Replace handleSocialClick / startSocialAuth in register
    old_google_block = """    if (provider === 'google' && !socialConfig.googleEnabled) {
      setError('Google registration is currently disabled by administrator.');
      return;
    }"""

    new_google_block = """    if (provider === 'google') {
      if (!socialConfig.googleEnabled) {
        setError('Google registration is currently disabled by administrator.');
        return;
      }
      if (socialConfig.googleClientId && typeof window !== 'undefined') {
        setSocialLoading('google');
        const redirectUri = `${window.location.origin}/api/auth/callback`;
        const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?` +
          `client_id=${encodeURIComponent(socialConfig.googleClientId)}` +
          `&redirect_uri=${encodeURIComponent(redirectUri)}` +
          `&response_type=code` +
          `&scope=${encodeURIComponent('openid email profile')}` +
          `&prompt=select_account` +
    ...
    if old_google_block in reg_code:
        reg_code = reg_code.replace(old_google_block, new_google_block)
        with open(register_page_path, 'w', encoding='utf-8') as f:
            f.write(reg_code)
        print(f"✓ Patched /register with Google OAuth account selector: {register_page_path}")

print("🚀 Account selection redirect configured successfully!")
