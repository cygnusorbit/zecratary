...
for c in candidates:
    if os.path.exists(c) and (
        os.path.exists(os.path.join(c, 'profile')) or 
...
# 2. Page code for Social Login Settings
social_page_code = """'use client';
...
# Install to both singular and plural directories to prevent route mismatch
target_subdirs = [
    os.path.join(app_dir, 'admin', 'social-login-setting'),
    os.path.join(app_dir, 'admin', 'social-login-settings')
]
...
for d in target_subdirs:
    ...
    with open(p, 'w', encoding='utf-8') as f:
        f.write(social_page_code)
    print(f"✓ Installed social settings page at: {p}")

# 3. Add link into Sidebar.tsx so it can be navigated to directly
sidebar_paths = [
...
if sidebar_path:
    ...
    if '/admin/social-login-setting' not in sidebar_content:
        # Inject right after /admin/payment
        needle = "href=\"/admin/payment\""
        if needle in sidebar_content:
            target_chunk = """<Link href="/admin/social-login-setting" className={navClass('/admin/social-login-setting')} title="Social Login">
            ...
            # Find closing tag of payment link
            idx = sidebar_content.find(needle)
            close_link_idx = sidebar_content.find("</Link>", idx)
            if close_link_idx != -1:
                insert_pos = close_link_idx + len("</Link>")
                sidebar_content = sidebar_content[:insert_pos] + "\n                " + target_chunk + sidebar_content[insert_pos:]
                with open(sidebar_path, 'w', encoding='utf-8') as f:
                    f.write(sidebar_content)
                print(f"✓ Injected Social Login link into {sidebar_path}")
