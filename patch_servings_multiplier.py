...
import {
  ...
  ShoppingCart, Timer, Edit3, Share2, CheckSquare, Square, Star, ExternalLink, Trash2, Save, Plus, CheckCircle2, Type, Lock, GripVertical, Check
} from 'lucide-react';
...
export default function SavedRecipesPage() {
  ...
  // Function to multiply ingredient quantities dynamically based on servings ratio
  const calculateScaledAmount = (baseAmount: any, baseServings: number, currentServings: number) => {
    ...
    // Format nicely (e.g. integer or up to 2 decimal places)
          ...
          <div className="bg-[#111726] border border-slate-800 rounded-3xl max-w-4xl w-full max-h-[92vh] flex flex-col overflow-hidden shadow-2xl relative">
            ...
            <div className="overflow-y-auto flex-1 space-y-6">
              ...
              {!isEditing ? (
                ...
                /* FULL EDIT FORM WITH MOUSE DRAG REORDER & DONE BUTTON */
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4 text-xs">
                      ...
                      {isReorderingIngredients && (
                        <div className="p-2.5 bg-emerald-950/40 border border-emerald-500/30 rounded-xl text-emerald-300 text-[11px] flex items-center gap-2">
                          <Check className="h-4 w-4 shrink-0" /> Drag items up or down using the grip handles on the right, then click <strong>Done</strong> when finished.
                        </div>
                      )}
                        ...
                        {Array.isArray(editForm.ingredients) && editForm.ingredients.map((ing: any, idx: number) => (
                          <div
                            ...
                            className={`flex items-center gap-2 bg-[#0B101D] p-2.5 rounded-xl border transition ${
                              isReorderingIngredients ? 'border-emerald-500/60 cursor-grab active:cursor-grabbing bg-[#111928]' : 'border-slate-800'
                            }`}
                          >
                            <input
                              type="text"
                            ...
                            {isReorderingIngredients ? (
                              <div className="p-2 text-emerald-400 cursor-grab" title="Drag to reorder">
                  ...
                  {editTab === 'steps' && (
                    <div className="space-y-4 text-xs">
                      ...
                      {isReorderingSteps && (
                        <div className="p-2.5 bg-emerald-950/40 border border-emerald-500/30 rounded-xl text-emerald-300 text-[11px] flex items-center gap-2">
                          <Check className="h-4 w-4 shrink-0" /> Drag steps up or down using the grip handles on the right, then click <strong>Done</strong> when finished.
                        </div>
                      )}
                        ...
                        {Array.isArray(editForm.instructions) && editForm.instructions.map((step: string, idx: number) => (
                          <div
                            ...
                            className={`flex items-start gap-3 bg-[#0B101D] p-3 rounded-xl border transition ${
                              isReorderingSteps ? 'border-emerald-500/60 cursor-grab active:cursor-grabbing bg-[#111928]' : 'border-slate-800'
                            }`}
                          >
                            ...
                            {isReorderingSteps ? (
                              <div className="p-2 text-emerald-400 cursor-grab mt-1" title="Drag to reorder">
                                <GripVertical className="h-4 w-4" />
                              </div>
                            ) : (
                              <button
...
print("✅ Servings multiplier scaling successfully integrated into recipe ingredients view!")
