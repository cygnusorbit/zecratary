...
export default function PlannerPage() {
  ...
  const handleAddMealSubmit = (e: React.FormEvent) => {
    ...
    if (!selectedRecipeObj) {
      alert('Please select a recipe from the list first.');
      return;
    }
    const newMeal = {
      id: 'plan_' + Date.now(),
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-3 rounded-2xl border cursor-pointer transition select-none ${
                            isSelected 
                              ? 'bg-[#161213] border-[#E05638] shadow-sm' 
...
print("✅ Planner Add Meal selection and state handling updated!")
