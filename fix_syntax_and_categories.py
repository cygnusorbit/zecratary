...
# 1. FIX PANTRY PAGE (Restoring broken selects)
pantry_path = "apps/web/src/app/pantry/page.tsx"
if os.path.exists(pantry_path):
    with open(pantry_path, "r", encoding="utf-8") as f:
        code = f.read()

    # Fix Add Modal Select
    code = re.sub(
        r'<select[\s\S]*?value=\{itemCategory\}[\s\S]*?onChange=\{\(e\) =>\s*<option value="Produce">.*?</select>',
        f'''<select
                  value={{itemCategory}}
                ...
                >{full_options_jsx}                </select>''',
        code
    )
    # Fix Edit Modal Select
    code = re.sub(
        r'<select[\s\S]*?value=\{editingItem\.category\}[\s\S]*?onChange=\{\(e\) =>\s*<option value="Produce">.*?</select>',
        f'''<select
...
# 2. FIX RECIPES PAGE (Restoring broken selects)
...
if os.path.exists(recipes_path):
    ...
    code = re.sub(
        r'<select[\s\S]*?value=\{ing\.category \|\| \'Pantry Staples\'\}[\s\S]*?onChange=\{\(e\) =>\s*<option value="Produce">.*?</select>',
        f'''<select
...
# 3. COMPLETELY REBUILD SHOPPING PAGE (Guarantees zero syntax errors)
shopping_code = f"""'use client';
import {{ useState, useEffect }} from 'react';
import Link from 'next/link';
import {{ ShoppingCart, Plus, Trash2, CheckSquare, Square, Star, Copy, Edit3, X, Save, Search, CheckCircle2 }} from 'lucide-react';
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto shrink-0">
          ...
          <button
            ...
            className={{`flex-1 sm:flex-initial border font-bold text-xs px-4 py-3 rounded-2xl transition flex items-center justify-center gap-2 shadow-sm ${{
              showStaplesOnly ? 'bg-amber-500/20 border-amber-500 text-amber-300' : 'bg-[#111726] hover:bg-[#1a2338] border-amber-900/80 text-amber-400'
            }}`}}
          >
...
with open("apps/web/src/app/shopping/page.tsx", "w", encoding="utf-8") as f:
    f.write(shopping_code)

print("✅ Build errors cleared! Categories and code structure fully restored across Shopping, Recipes, and Pantry.")
