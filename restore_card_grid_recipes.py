...
export default function RecipesPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 text-slate-100 pb-20 px-2 sm:px-4">
      ...
      {/* CREATE RECIPE MODAL WITH DYNAMIC 20 CATEGORIES */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                ...
                {formIngredients.map((ing, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="bg-[#0B101D] border border-slate-800 rounded-xl p-2.5 text-slate-300 outline-none text-xs"
                    >
                      {CATEGORIES.map(cat => (
                        <option key={cat} value={cat}>{cat}</option>
                      ))}
...
print("✅ Saved Recipes page successfully restored to your exact card-grid layout!")
