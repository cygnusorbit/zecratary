...
# 1. Inspect recipes page to ensure exact storage key match
recipes_path = "apps/web/src/app/recipes/page.tsx"
storage_key = "zecratary_recipes"

if os.path.exists(recipes_path):
    with open(recipes_path, "r", encoding="utf-8") as f:
        recipes_content = f.read()
    match = re.search(r"localStorage\.(?:getItem|setItem)\(['\"]([^'\"]+)['\"]", recipes_content)
    if match:
        storage_key = match.group(1)
        print(f"🔍 Detected active Saved Recipes storage key: '{storage_key}'")

# 2. Re-write planner page with exact synchronized storage handler
...
export default function PlannerPage() {{
  ...
  const loadAllSavedRecipes = () => {{
    if (typeof window === 'undefined') return;
    
    // Check primary key first, then fallbacks
    const keysToCheck = ['{storage_key}', 'zecratary_recipes', 'saved_recipes', 'recipes'];
    let loaded: any[] = [];
    ...
    if (loaded.length === 0) {{
      // Align with exact default list from Saved Recipes page
      loaded = [
        {{
          id: 'rec_1',
  ...
  useEffect(() => {{
    ...
    const handleStorageChange = () => loadAllSavedRecipes();
...
print(f"✅ Planner successfully updated and bound to active key: '{storage_key}'")
