...
import { ChefHat, BookOpen, UploadCloud, Calendar, ShoppingCart, Settings, PlusCircle, Package, ShieldCheck } from 'lucide-react';
import './globals.css';
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold text-xs transition ${
                        active
...
print("✅ Sidebar navigation successfully updated with Admin Setting!")
