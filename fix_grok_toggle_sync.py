...
# 1. UPDATE lib/socialAuth.ts WITH STRICT BOOLEAN PARSING
...
print(f"✓ Updated {social_auth_path} with strict boolean handling.")

# 2. UPDATE /admin/social-login-setting/page.tsx WITH INSTANT AUTO-PERSIST ON TOGGLE
admin_page_path = os.path.join(app_dir, 'admin', 'social-login-setting', 'page.tsx')
if os.path.exists(os.path.dirname(admin_page_path)):
...
export default function AdminSocialLoginSettingPage() {
  ...
  const handleSave = (e?: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMsg({ 
        text: 'Social Login Settings updated and synchronized across all pages!', 
        type: 'success' 
      });
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs" style={{ color: isDayMode ? '#64748b' : 'var(--color-text-secondary, #94a3b8)' }}>
            Configure Google, Facebook, and Grok OAuth providers. Toggling instantly synchronizes with /register.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: isDayMode ? '#e2e8f0' : '#1e293b' }}>
              <span className="font-black text-sm" style={{ color: isDayMode ? '#0f172a' : '#ffffff' }}>Google OAuth</span>
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: isDayMode ? '#e2e8f0' : '#1e293b' }}>
              <span className="font-black text-sm" style={{ color: isDayMode ? '#0f172a' : '#ffffff' }}>Facebook Login</span>
              <label className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                <input 
                  type="checkbox"
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: isDayMode ? '#e2e8f0' : '#1e293b' }}>
              <span className="font-black text-sm" style={{ color: isDayMode ? '#0f172a' : '#ffffff' }}>xAI Grok</span>
              <label className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                <input 
                  type="checkbox"
...
# 3. UPDATE /register WITH BULLETPROOF STRICT EVALUATION
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('zecratary_social_config_updated', syncSocialConfig);
    ...
    return () => {
      ...
      window.removeEventListener('zecratary_social_config_updated', syncSocialConfig);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* DYNAMIC BUTTONS - STRICTLY GATED */}
        {hasAnySocial && (
...
# Apply to all register routes
register_candidates = [
    os.path.join(app_dir, 'register', 'page.tsx'),
    os.path.join(app_dir, '(auth)', 'register', 'page.tsx')
]

for p in register_candidates:
    if os.path.exists(os.path.dirname(p)):
        with open(p, 'w', encoding='utf-8') as f:
            f.write(register_code)
        print(f"✓ Patched register page at: {p}")
