...
import {
  ...
  ShoppingCart, Timer, Edit3, Share2, CheckSquare, Square, Star, ExternalLink, Trash2, Save, Plus, CheckCircle2, Type
} from 'lucide-react';
...
export default function SavedRecipesPage() {
  ...
  // Reference UI States: Font Size Scale (80% to 140%) & Checked Steps Tracking
          ...
          setRecipes([
            {
              ...
              ingredients: [
                { item: '35 g palm sugar, chopped (3 tbsp tightly packed)', quantity: '' },
                { item: '3 tbsp (45 ml) water', quantity: '' },
                { item: '3-4 tbsp thai cooking tamarind', quantity: '' },
                { item: '2 tbsp good fish sauce', quantity: '' },
                { item: '4 oz (115g) dry rice noodles, medium size', quantity: '' },
                { item: '2 tbsp dried shrimp, roughly chopped', quantity: '' },
                { item: '3 cloves garlic, chopped', quantity: '' },
                { item: '1/4 cup roughly chopped shallots', quantity: '' },
                { item: '3 oz (85 g) pressed tofu, cut into small pieces', quantity: '' },
                { item: '3 tbsp finely chopped sweet preserved daikon radish', quantity: '' },
                { item: 'Dried chili flakes, to taste', quantity: '' },
                { item: '3 tbsp (45 ml) vegetable oil', quantity: '' },
                { item: '10 medium sized shrimp', quantity: '' },
                { item: '2 eggs', quantity: '' },
                { item: '2 1/2 cup (120 g) bean sprouts, loosely packed', quantity: '' },
                { item: '7-10 stalks (70 g) garlic chives, cut into 2" pieces', quantity: '' },
                { item: '1/4 cup roasted peanuts, roughly chopped', quantity: '' },
                { item: '1 lime', quantity: '' }
              ],
              instructions: [
                ...
                'Heat oil in a wok over high heat, toss in garlic, shallots, and preserved radish until fragrant.',
                'Add shrimp and tofu, stir-frying until cooked through, then push to the side of the wok.',
                'Crack eggs into the empty space, let set slightly, then scramble gently.'
              ]
            }
          ]);
        }
            ...
            <div className="overflow-y-auto flex-1 space-y-6">
              ...
              {!isEditing ? (
                <>
                  ...
                  {/* ───────────────────────────────────────────────────────────── */}
                  {/* REFERENCE-MATCHED INGREDIENTS & INSTRUCTIONS SECTION WITH FONT ENLARGE */}
                  {/* ───────────────────────────────────────────────────────────── */}
                  <div className="px-6 space-y-8">
                    
                    {/* Ingredients Header with Font Enlarge Controls */}
                    <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                      <h3 className="text-xl font-black text-white tracking-wide">Ingredients</h3>
                      
                      {/* Font Size Adjuster Control (Matching Reference Screenshot) */}
                    ...
                    {/* Two-Column Ingredients Layout with Orange Dot Bullets */}
                    <div
                        ...
                        return (
                          <div key={i} className="flex items-start gap-3 py-1">
                            ...
                            <span className="text-slate-200 leading-snug">
                              {ingText} {qtyText && <strong className="text-white font-semibold">({qtyText})</strong>}
                            </span>
                          </div>
                        );
                      })}
                    ...
                    {/* Instructions Section matching Reference Layout & Checkboxes */}
                  ...
                  {editTab === 'ingredients' && (
                    <div className="space-y-4 text-xs">
                      <div className="flex justify-between items-center">
                        <label className="font-bold text-[#E05638] uppercase">Recipe Ingredients</label>
                        <button
                          onClick={() => {
                          ...
                          className="bg-[#E05638] text-white font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 text-[11px]"
                        >
                      ...
                      <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                        {Array.isArray(editForm.ingredients) && editForm.ingredients.map((ing: any, idx: number) => (
                          <div key={idx} className="flex items-center gap-2 bg-[#0B101D] p-2 rounded-xl border border-slate-800">
                            <input
                              type="text"
                            ...
                            <input
                              ...
                              value={ing.quantity || ing.amount || ''}
                              onChange={(e) => {
                                const list = [...editForm.ingredients];
                                list[idx] = { ...list[idx], quantity: e.target.value };
                                setEditForm({ ...editForm, ingredients: list });
                              }}
                              className="w-28 bg-slate-900 border border-slate-800 rounded-lg p-2 text-xs text-emerald-400 font-bold outline-none"
                            />
                            <button
                              onClick={() => {
...
print("✅ Recipe layout upgraded to match reference: 2-column orange dot ingredients, font scaler, and checkmark steps!")
