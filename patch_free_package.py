...
# -------------------------------------------------------------
# 1. API Route: Bypass Stripe if Price is 0 or Free Tier
# -------------------------------------------------------------
...
export async function POST(req: Request) {
  try {
    ...
    // Check if package is Free or bypass is requested
    const isFreeTier = isFree || priceCents === 0;

    // Only interact with Stripe if it's a paid tier AND Stripe API key is configured
...
# -------------------------------------------------------------
# 2. Package Manager UI (/admin/plans) with Free Tier Bypass Toggle
# -------------------------------------------------------------
files["apps/web/src/app/admin/plans/page.tsx"] = """'use client';
import { useState, useEffect } from 'react';
import { PlusCircle, PackageCheck, Zap, ShieldCheck, Check, Sparkles } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-8 text-slate-100">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Create free and premium packages with customizable quotas. Free plans bypass Stripe automatically.
        </p>
      </div>
        ...
        <form onSubmit={handleCreatePlan} className="bg-[#111726] border border-slate-800 p-6 rounded-2xl space-y-4 md:col-span-1">
          ...
          {!isFree ? (
            ...
            <div className="p-3 bg-emerald-950/40 border border-emerald-500/20 rounded-xl text-xs text-emerald-300">
              🌱 <strong>Zero-Cost Package:</strong> Bypasses Stripe checkout. Users can activate this package immediately.
            </div>
          )}
          ...
          <div className="space-y-3">
            {plans.length === 0 ? (
              <div className="text-slate-500 text-xs py-8 text-center bg-[#0B101D] rounded-xl border border-slate-800">
                No subscription packages in database yet. Create your first free package above!
              </div>
            ) : (
              plans.map((p) => (
                <div
...
print("✅ Free package feature with Stripe bypass installed successfully!")
